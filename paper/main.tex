...
\author[1]{Kai Zhao}
\author[1]{Xu Chu}
\affil[1]{University of Florida, Gainesville, FL, USA}
...
We deliberately do not claim a hardware \emph{improvement} for the neural mitigator. On this device the four-qubit unmitigated error sits at the shot-noise floor (mean absolute error $0.013$ at $8{,}192$ shots), leaving little systematic bias to correct, and a model trained on an Aer digital twin of the device \emph{overcorrects} when deployed on the physical processor---a sim-to-real distribution shift, since the twin's depolarizing model overstates the device's actual bias. We report this honestly as a sim-to-real study in Appendix~\ref{app:hardware}; closing the gap requires on-device or data-augmented training~\citep{liao2025noiseagnostic,adeniyi2025adaptive}, which we leave to future work. The hardware contribution here is the device-level validation of the ZNE failure mode, the paper's central and most citable finding.
