...
% Unnumbered footnote for the code/data link on page 1.
...
Variational quantum algorithms such as the variational quantum eigensolver (VQE) and the quantum approximate optimization algorithm (QAOA) are leading candidates for near-term quantum advantage, but their expectation-value estimates are biased by device noise \citep{preskill2018quantum,cerezo2021variational,bharti2022noisy,tilly2022variational}. Error mitigation aims to remove this bias without the qubit overhead of error correction \citep{temme2017error,cai2023quantum}. The most widely used technique, zero-noise extrapolation, amplifies noise by gate folding and extrapolates the measurements back to the zero-noise limit \citep{temme2017error,li2017efficient,giurgicatiron2020digital,kim2023evidence}. Learning-based methods such as Clifford data regression instead fit a noisy-to-ideal map on classically simulable circuits run on the same device \citep{czarnik2021error,strikis2021learning}.

These methods are usually compared at a fixed, generous number of shots \emph{per circuit execution}. That comparison hides a first-order practical distinction: the methods need very different numbers of executions per estimate. ZNE needs three to five per mitigated value. CDR runs one execution per training circuit---more than thirty---for every new target circuit. A neural model trained offline needs exactly one. Inside a VQA loop that evaluates thousands of parameter settings under a per-iteration shot budget, these multiplicities decide which method is usable, not the accuracy at unlimited shots. \citet{bultrini2023unifying} made the budget axis explicit for classical methods and found the ranking is budget-dependent; we extend that analysis to learned mitigation and make the budget an input of the model itself.
...
\emph{(ii) An equal-budget evaluation protocol and Pareto analysis.} Every method receives the same total shot budget $B$ per evaluation point, split across the executions it requires, with every execution---including each ZNE fold point and each CDR training circuit---drawn as measured shots under the same frozen noise realization (Section~\ref{sec:setup}). Under this protocol NEM is the most accurate method at every budget under coherent miscalibration, and the only method that beats unmitigated execution at $B\le 2^{14}$ under stochastic device noise; CDR overtakes it only at $B=2^{16}$ (Section~\ref{sec:pareto}).

\emph{(iii) A characterization of extrapolation failure.} Under physically motivated control-offset miscalibration \citep{krantz2019engineer}, unitary folding amplifies the coherent angle error while the observable responds non-monotonically. ZNE then underperforms unmitigated execution on 38--63\% of instances, with unbounded single-instance error. We map this failure boundary against the miscalibration strength, quantify the tail risk (Section~\ref{sec:failure}), and confirm the failure on a real IBM Heron processor (Section~\ref{sec:hardware}).

\emph{(iv) Scaling and controls.} Mean error reductions of 61--85\% under structured noise persist without degradation from 4 to 20 qubits, and a direct-prediction control---predicting $\Hideal$ from features alone, without the noisy measurement---never beats unmitigated execution at any size, confirming that mitigation, not memorization, drives the result (Sections~\ref{sec:scaling} and~\ref{sec:ablations}).
...
We condition training on the shot-noise scale so that one model serves every deployment budget. Each training example draws its shot count $S$ log-uniformly from $2^{8}$ to $2^{16}$, samples the noisy input at that $S$, and appends $1/\sqrt{S}$ to $\mathbf{x}_n$. The network thereby learns how much of the input fluctuation is measurement noise versus learnable bias---at small $S$ it shrinks its trust in the measurement, at large $S$ it corrects fine structure---so no retraining is needed across budgets.
...
The offline cost is recovered after a modest number of evaluations. Let $M$ be the number of distinct evaluation points---parameter settings, or target circuits---that a deployed model serves before retraining. Counting every execution at equal per-execution shots, NEM's total quantum cost is $4400 + M$, against $kM$ for ZNE ($k=3$ or $4$) and $32M$ for CDR. The baselines carry no offline cost, but they pay their multiplicity at \emph{every} point: CDR, in particular, re-fits its near-Clifford regression for each target circuit. NEM is cheaper in total once $M$ exceeds $M^\star = \lceil 4400/(k-1) \rceil$: 142 points relative to CDR, 1{,}467 relative to adaptive ZNE, and 2{,}200 relative to exponential ZNE (Table~\ref{tab:breakeven}). A VQE optimization loop evaluates $10^3$--$10^4$ parameter settings, so it clears the CDR break-even by one to two orders of magnitude within a single run. The bound is conservative: training shots are drawn from $2^8$--$2^{16}$, so a deployment budget $B>2^8$ per execution amortizes the offline cost faster still. This break-even counts executions at equal per-execution shots, whereas the equal-budget protocol of Section~\ref{sec:setup} fixes the total shots per evaluation; the two are complementary accountings of the same quantum cost---one asks how many evaluations repay the offline cost, the other which method is most accurate at a fixed per-evaluation budget.
...
\caption{Equal-budget comparison at $n=8$ (100 instances; median $|$error$|$, interquartile range shaded). Every method spends the same total budget $B$ per evaluation point, split across its required executions (Table~\ref{tab:cost}). NEM is the most accurate method at every budget under miscalibration and the only one below the unmitigated baseline at $B\le2^{14}$ under stochastic noise; CDR crosses below NEM only at the largest budget. ZNE medians track the unmitigated line: for the typical instance extrapolation removes nothing, while its mean error diverges (Section~\ref{sec:failure}).}
\label{fig:pareto}
\end{figure}

Under equal-budget comparison, NEM dominates at low-to-moderate budgets and is overtaken only at the largest budget we test. Figure~\ref{fig:pareto} shows this; Table~\ref{tab:budget_full} in the appendix lists every value. Under coherent-miscalibration noise, NEM reduces the mean error by 72\%, 77\%, 81\%, and 81\% at $B=2^{10},2^{12},2^{14},2^{16}$ respectively---flat in $B$, because the learned correction targets the systematic bias and its training cost is already amortized. CDR, whose per-execution shot count is $B/32$, climbs from $+23\%$ at $B=2^{10}$ to $+84\%$ at $B=2^{16}$, overtaking NEM only at the largest budget. Both ZNE variants are \emph{negative} at essentially every budget in this regime ($-36\%$ to $-471\%$ in the mean); the lone exception---adaptive ZNE at $B=2^{14}$ ($+7\%$ mean)---is a heavy-tail artifact whose median still sits at the unmitigated level. Under stochastic device noise the picture compresses (the residual error is dominated by shot noise), but the ordering at low budget is the same: NEM is the only positive method at $B\le2^{14}$ ($+11$ to $+20\%$), and CDR turns positive only at $B=2^{16}$ ($+47\%$).

The practical reading is direct. A VQE optimization loop evaluates $10^3$--$10^4$ parameter settings at a per-iteration budget of $10^3$--$10^4$ shots. That regime is exactly where single-execution learned mitigation is the most reliable option---the only one that helps under stochastic noise, and the most accurate under miscalibration; per-instance methods match it on accuracy only when the per-evaluation budget grows by more than an order of magnitude.
...
\caption{Median $|$error$|$ versus miscalibration strength $\delta$ at $n=8$ (80 instances per point, 8{,}192 shots per execution). ZNE medians coincide with the unmitigated baseline at every $\delta$, including $\delta=0$, where the residual bias is dominated by readout error that folding does not amplify. NEM matches CDR at small $\delta$ at $1/32$ the quantum cost, and its improvement declines smoothly as the unlearnable coherent share grows. A single NEM is trained once on $\delta\sim\mathcal{U}(0,0.09)$---wider than the $[0.02,0.06]$ range used in the other experiments---and evaluated across the sweep, so every point is in-distribution and no model is retrained per~$\delta$.}
\label{fig:boundary}
\end{figure}

Figure~\ref{fig:boundary} sweeps the coherent bias $\delta$ from 0 to 0.08\,rad over a fixed stochastic background, and the methods separate cleanly. ZNE's \emph{median} improvement never exceeds $+2\%$ anywhere on the sweep---including at $\delta=0$, because the remaining bias there is dominated by readout error, which gate folding does not amplify; extrapolation removes the gate-error component and faithfully retains the rest.

Its \emph{mean} error, however, explodes as $\delta$ grows (for exponential extrapolation, $-646\%$ at $\delta=0.06$), driven by heavy tails. Table~\ref{tab:safety} shows the anatomy at one representative cell, and Table~\ref{tab:safety_full} in the appendix shows every structured-regime cell: ZNE returns a result worse than the unmitigated estimate on 38--63\% of instances, with single-instance errors reaching three orders of magnitude above the unmitigated level when the extrapolation fit diverges. Extrapolation is an unbounded operation: at $n{=}12$ we observe a single instance where the adaptive variant's error reaches $371$ in observable units, more than three orders of magnitude above the worst unmitigated error in that cell.

NEM, by contrast, tracks CDR at small $\delta$ ($+82\%$ versus $+83\%$ mean at $\delta=0$) at $1/32$ of the quantum cost. Its improvement then declines smoothly to $+44\%$ at $\delta=0.08$---still inside its $[0,0.09]$ training range, so this is genuine learning difficulty, not extrapolation---as the high-dimensional oscillatory component of the coherent bias exceeds what the network can learn from 4{,}000 samples. CDR, which re-fits per instance, stays at 69--86\%.
...
The asymmetry in failure behavior matters as much as the averages. The neural model's output is a bounded residual correction: across all 2{,}000 structured-regime test instances (4--20 qubits) its worst-case error stays \emph{below} the worst unmitigated error, in every cell.

This bounded behavior can be tightened further with a safeguard. We train three seeds and fall back to the raw value when they disagree beyond a threshold calibrated on unlabeled data. This drops the worse-than-raw rate from 7\% to 5\% at a cost of three points of mean improvement, and in the low-noise stochastic regime it pins the model's worst case exactly at the unmitigated level. We view the safeguard as cheap insurance rather than a core mechanism.

\subsection{Real-hardware validation}
\label{sec:hardware}

The failure analysis above is the paper's most consequential claim, so we test it on a real device. We run the hardware-efficient VQE on \texttt{ibm\_marrakesh}, a 156-qubit Heron~r2 superconducting processor, using four qubits at two circuit depths (2 and 5 ansatz layers). We compare unmitigated execution against folding ZNE under the equal-budget protocol, with ten and eight test instances respectively, 8{,}192 shots, and all jobs grouped in a single batch. Real devices carry intrinsic control-offset miscalibration, so no noise is injected---the device \emph{is} the noise source.
...
This run validates the failure analysis; the neural mitigator's headline improvements are established in simulation, where training and test share the noise process. On hardware, a model trained purely on a digital twin shows a simulation-to-hardware gap (Appendix~\ref{app:hardware}). We attribute this gap primarily to the twin's mismatch rather than to learned mitigation itself---consistent with the affine fit below recovering a shrinkage factor below one (the twin overcorrects)---though with a single device we cannot fully separate the two. Fine-tuning the twin-trained model on a small on-device set---120 four-qubit circuits measured in a single batched job, labelled by exact classical simulation---raises its held-out error reduction on \texttt{ibm\_marrakesh} from $+16\%$ (twin-only) to $+39\%$ on instances from the same calibration epoch, a gain stable across hyperparameters and seeds, and a deterministic affine recalibration of the twin's correction reaches $+45\%$ on this epoch (Appendix~\ref{app:hardware})~\citep{liao2025noiseagnostic,adeniyi2025adaptive}. Unlike ZNE, whose folding amplifies coherent error without bound, the learned correction is bounded and beneficial in aggregate once a small amount of same-epoch device data is supplied---though still worse than unmitigated on about a quarter of individual instances, and calibration-specific, as Appendix~\ref{app:hardware} details.
...
NEM's improvement does not degrade as circuits grow from 4 to 20 qubits. Figure~\ref{fig:scaling} shows this; Table~\ref{tab:scaling_full} in the appendix lists every cell. Mean error reduction under coherent miscalibration is 72\%, 77\%, 80\%, 85\%, 76\%, 80\%, 77\% at $n=4,6,8,10,12,16,20$, and the stochastic-correlated regime is similarly flat (61--68\%); in both structured regimes every NEM improvement is significant ($p<10^{-4}$, paired $t$-test), with no degradation up to the largest size simulated. At $n{=}16$ under miscalibration, NEM's $+80\%$ matches CDR's $+81\%$, and at $n{=}20$ its $+77\%$ matches CDR's $+79\%$, each at $1/32$ of the quantum cost. The per-size models use identical architecture and training budgets; only the input dimension grows, linearly in $n$.

Under purely stochastic noise the headroom itself shrinks: the unmitigated error approaches the shot-noise floor of the per-execution budget ($S{=}8{,}192$ up to $n{=}10$; $1{,}024$ at $n{=}12,16$; $512$ at $n{=}20$, chosen for simulation cost). NEM's improvement then tracks that ceiling: $+10$ to $+22\%$ ($p<0.05$) through $n=12$, $+7\%$ ($p=0.12$, not significant) at $n=16$, and $+11\%$ ($p=0.026$) at $n=20$. More than half of the residual error there is irreducible measurement noise, so little bias is left for any single-execution method to remove.

\subsection{Controls and ablations}
\label{sec:ablations}

\paragraph{Direct prediction.}
A skeptical reader should ask whether the network is mitigating or merely memorizing the expectation-value landscape. A control model with identical architecture but no access to the noisy measurement answers this: it is worse than unmitigated execution in every cell---never beating it at any size or regime, by as much as $927\%$ (Table~\ref{tab:scaling_full}). The measurement is indispensable; the network's role is correction, not regression of $\Hideal$ from circuit parameters.
...
Several limitations bound these results and mark the directions we see as most promising. Our evaluation is primarily in simulation; we validate the ZNE failure mode on real hardware (Section~\ref{sec:hardware}), and the neural mitigator's headline improvements are established in simulation. A model trained purely on a digital twin shows a simulation-to-hardware gap, which a small on-device fine-tuning set closes (Appendix~\ref{app:hardware}); the fitted correction is specific to a calibration epoch and would be refreshed periodically in deployment, as device-data methods such as CDR already require. The noise models, while physically motivated (calibration offsets, crosstalk, readout asymmetry), are otherwise not device-fitted. Training labels require exact classical simulation, restricting training to classically simulable sizes; transfer from small training circuits to larger deployment circuits \citep{cantori2024synergy,strikis2021learning} and label-free data augmentation \citep{liao2025noiseagnostic} are the natural extensions for further reducing the device data the on-device fine-tuning above requires, and both compose with our budget conditioning. Our observables are diagonal ($Z$-type); non-diagonal Hamiltonians require basis-rotated measurement and may change feature design. Each model is trained per device family and noise regime; drift beyond the characterized feature ranges requires retraining, though the success of shot-budget conditioning suggests broader conditioning is feasible. Finally, our ansatz family is fixed; generalization across circuit families is untested here and is where graph- or sequence-structured encoders would plausibly earn their complexity.
...
\input{tables/budget_full}
\input{tables/safety_full}

\section{Hardware sim-to-real study}
\label{app:hardware}

Section~\ref{sec:hardware} validates the ZNE failure mode on \texttt{ibm\_marrakesh} and notes that a purely twin-trained neural mitigator shows a simulation-to-hardware gap. We give the full picture here---first the gap under the hardest, no-device-data setting, then the small amount of on-device data that closes it---because both halves are informative about when and how learned mitigation can be deployed.
...
Two considerations qualify the negative result. The gap is a property of the \emph{twin}, not the method: in simulation, where training and test share the noise process (the deployment setting CDR and on-device methods also assume), the model is strongly positive (Sections~\ref{sec:pareto}--\ref{sec:scaling}). The hardware experiment swaps a matched noise model for a mismatched one, and the model degrades accordingly---the expected behaviour of a supervised corrector under covariate shift. The failure is also bounded in principle: the model emits a residual correction, and the ensemble safeguard of Section~\ref{sec:failure} gates large-disagreement corrections back to the raw value, so a safeguarded deployment would track the unmitigated baseline rather than ZNE's unbounded amplification.

\paragraph{Closing the gap with on-device fine-tuning.}
The gap closes with a small amount of device data. We measure a fine-tuning set of 120 random four-qubit, five-layer circuits on \texttt{ibm\_marrakesh} in a single batched job, compute their exact ideal labels by classical statevector simulation, and fine-tune the twin-trained model on these real (noisy, ideal) pairs. We evaluate on 16 held-out instances measured in the same job, so training and test share one calibration epoch---the setting a deployment actually faces. Table~\ref{tab:hardware_ft} reports the outcome. On this draw the twin-only model already improves by $+16\%$ over unmitigated execution; fine-tuning lifts it to $+39\%$, and halves the worse-than-unmitigated rate from $50\%$ to $25\%$. The single best-seed run reaches $+56\%$; we report the more conservative mean over all settings. The gain is stable, not a lucky configuration: across $18$ learning-rate/epoch/encoder-freezing settings, each run over five seeds ($90$ runs), every configuration mean is positive (range $+27\%$ to $+41\%$) and the worst individual seed still reduces error by $+20\%$. A two-parameter affine recalibration of the twin's correction, $\langle H\rangle_{\mathrm{pred}}=\langle H\rangle_{\mathrm{noisy}}+a\,(\langle H\rangle_{\mathrm{twin}}-\langle H\rangle_{\mathrm{noisy}})+b$ fitted on the same device set, reaches $+45\%$ with $a=0.77<1$---a direct measurement that the twin overcorrects, exactly as the negative result diagnosed.

We are explicit about the scope of this correction: it is specific to a calibration epoch. The twin-only and fine-tuned models above are compared \emph{within} one epoch---training and held-out circuits measured in a single job---which is the comparison a deployment faces. Across epochs the fit does not transfer: applying this epoch's fine-tuned model to the earlier five-layer instances of Table~\ref{tab:hardware} (recomputed from their recorded measurements; the script is in the repository) yields a smaller improvement than the current twin, because the device has drifted and those near-shot-noise circuits leave little headroom. On those same instances the \emph{current} twin checkpoint scores $+39.5\%$, so Table~\ref{tab:hardware}'s $-94\%$ reflects an earlier twin and calibration rather than intrinsic instability of the method; we therefore report the within-epoch gain only. This epoch dependence is shared with every device-data method: CDR re-fits on freshly drawn device circuits for each target circuit, paying the cost per evaluation, whereas a fine-tuned model amortizes one device-data fit across an entire evaluation loop and is refreshed only when calibration drifts. A small, classically labelled on-device set is thus enough to turn the learned mitigator from a marginal corrector into a clearly beneficial one within an epoch---the route the negative result pointed to~\citep{liao2025noiseagnostic,adeniyi2025adaptive}.

\begin{table}[t]
\caption{On-device fine-tuning on \texttt{ibm\_marrakesh} (4 qubits, 5 layers, $8{,}192$ shots, 16 held-out instances; unmitigated MAE $.018$). The twin-only model is trained on the digital twin alone; the fine-tuned model adds 120 device-measured circuits with exact classical labels, and all three of its cells (improvement, worse-rate, MAE) are means over $18$ settings $\times$ $5$ seeds, every configuration positive; the affine row recalibrates the twin's correction on the same device set. All rows are evaluated within one calibration epoch (Appendix text). ``Worse'' is the fraction of instances exceeding the unmitigated error.}
\label{tab:hardware_ft}
\centering
\small
\begin{tabular}{lccc}
\toprule
Method & Improvement & Worse & MAE \\
\midrule
NEM (twin-only) & $+16\%$ & 50\% & .015 \\
NEM (on-device fine-tuned) & $+39\%$ & 25\% & .011 \\
NEM (affine recalibration) & $+45\%$ & 19\% & .010 \\
\bottomrule
\end{tabular}
\end{table}
