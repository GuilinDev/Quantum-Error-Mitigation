...
\emph{(iii) A characterization of extrapolation failure.} Under physically motivated control-offset miscalibration \citep{krantz2019engineer}, unitary folding amplifies the coherent angle error while the observable responds non-monotonically. ZNE then underperforms unmitigated execution on 38--65\% of instances, with unbounded single-instance error. We map this failure boundary against the miscalibration strength, quantify the tail risk (Section~\ref{sec:failure}), and confirm the failure on a real IBM Heron processor (Section~\ref{sec:hardware}).
...
Third, NEM tracks CDR at small $\delta$ ($+82\%$ versus $+83\%$ mean at $\delta=0$) at $1/32$ of the quantum cost. Its improvement then declines smoothly to $+44\%$ at $\delta=0.08$, as the high-dimensional oscillatory component of the coherent bias exceeds what the network can learn from 4{,}000 samples. CDR, which re-fits per instance, stays at 69--86\%.
